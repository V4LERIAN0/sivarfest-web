...
export default async function NewEventPage({ params }: { params: Promise<{ competitionId: string }> }) {
  ...
  const [competition, categories, athletes] = await Promise.all([
    getAdminCompetitionById(id),
    getAdminCategories(id),
    getAdminAthletes(id),
  ]);
  ...
  return <div className="max-w-4xl">
    ...
    <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><EventForm categories={categories} athletes={athletes} action={action} submitLabel="Create Event" /></div>
  </div>;
}
