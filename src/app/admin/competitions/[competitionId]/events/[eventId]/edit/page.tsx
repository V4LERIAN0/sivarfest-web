...
export default async function EditEventPage({ params }: { params: Promise<{ competitionId: string; eventId: string }> }) {
  ...
  const [event, categories, athletes] = await Promise.all([
    getAdminEvent(eventIdNumber),
    getAdminCategories(competitionIdNumber),
    getAdminAthletes(competitionIdNumber),
  ]);
  ...
  return <div className="max-w-4xl">
    ...
    <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/70 p-6"><EventForm event={event} categories={categories} athletes={athletes} action={action} submitLabel="Save Changes" /></div>
  </div>;
}
